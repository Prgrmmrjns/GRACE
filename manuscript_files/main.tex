...
An important aspect of this graph-based approach is the handling of features that participate in multiple disease mechanisms. When a feature is assigned to more than one group, it acts as a bridge, allowing it to interact with features from each of its assigned groups. For instance, if Feature B is in a group with Feature A and also in a group with Feature C, the resulting interaction constraints will be `[['A', 'B'], ['B', 'C']]`. This allows the model to learn interactions between A and B, and between B and C, but critically, it prevents a direct interaction between A and C. This mirrors the structure of the knowledge graph, where interactions are limited to connected nodes, preventing the model from exploring biologically implausible or distant relationships. This targeted approach is illustrated in Figure \ref{fig:multi_constraint_effect}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{images/multi_constraint_effect.png}
    \caption{Illustration of how a feature present in multiple interaction constraints is handled. Feature B, being part of two groups, can interact with both A and C, but A and C cannot interact directly.}
    \label{fig:multi_constraint_effect}
\end{figure}
